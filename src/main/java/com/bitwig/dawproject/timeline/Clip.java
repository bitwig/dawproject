...
/**
 * A Clip provides a clipped view on to a Timeline, and is used either on a Clips timeline
 * (typically for arrangements) or inside a ClipSlot element (for clip launcher Scenes). A Clip must
 * either have a child-element inheriting from Timeline or provide a ID reference to a timeline
 * somewhere else (for linked/alias clips).
 */
...
public class Clip extends Nameable
{
    ...
    /**
     * The TimeUnit used by the scope inside this timeline. This affects the content/reference,
     * playStart, playStop, loopStart, loopEnd but not time and duration which are using the
     * TimeUnit of the parent scope.
     */
    @XmlAttribute(required = false)
    ...
    /**
     * Reference to a Content Timeline this clip is playing, in case of linked/alias clips. You can
     * use either content or reference for one clip, but not both.
     */
    @XmlAttribute(required = false)
    @XmlIDREF
