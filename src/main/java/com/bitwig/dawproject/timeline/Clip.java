 ...
 * Clips timeline (typically for arrangements) or inside a ClipSlot element (for
 * clip launcher Scenes). A Clip must either have a child-element inheriting
...
public class Clip extends Nameable {
	...
	/**
	 * The TimeUnit used by the scope inside this timeline. This affects the
	 * content/reference, playStart, playStop, loopStart, loopEnd but not time and
	 * duration which are using the TimeUnit of the parent scope.
	 */
	@XmlAttribute(required = false)
	public TimeUnit contentTimeUnit;

	/**
	 * Time inside the content timeline (or reference) where the clip starts
	 * playing.
	 */
	@XmlAttribute(required = false)
	...
	/**
	 * Reference to a Content Timeline this clip is playing, in case of linked/alias
	 * clips. You can use either content or reference for one clip, but not both.
	 */
	@XmlAttribute(required = false)
	@XmlIDREF
